...
RES_FILE="$LOG_DIR/02_vessel_det.log"
mkdir -p $LOG_DIR

if [[ $CKPT_DIR == *"-int_"* ]]; then
  QUESTION_FILE=datasets/VLM_MVI/set1/vessel_seg/int_question_test.jsonl
else
  QUESTION_FILE=datasets/VLM_MVI/set1/vessel_seg/question_test.jsonl
fi
IMAGE_FOLDER=datasets/VLM_MVI/set1/vessel_seg/images
ANSWER_FILE="$LOG_DIR/z02_vessel_det.jsonl"
VIS_DIR="$LOG_DIR/.vis/02_vessel_det"
