...
RES_FILE="$LOG_DIR/04_NuCLS_det.log"
mkdir -p $LOG_DIR

# ********************************NuCLS********************************
if [[ $CKPT_DIR == *"-int_"* ]]; then
  QUESTION_FILE=datasets/nucleus_segment/NuCLS/detcls_int_question_NuCLS_test.jsonl
else
  QUESTION_FILE=datasets/nucleus_segment/NuCLS/detcls_question_NuCLS_test.jsonl
fi
IMAGE_FOLDER=datasets/nucleus_segment/NuCLS/images
ANSWER_FILE="$LOG_DIR/z04_NuCLS_det.jsonl"
VIS_DIR="$LOG_DIR/.vis/04_NuCLS_det"
