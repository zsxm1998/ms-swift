...
RES_FILE="$LOG_DIR/18_GBM_MGMT.log"
mkdir -p $LOG_DIR

# ************************************* Evaluate for Patch *************************************
QUESTION_FILE=datasets/GBM_MGMT/patch_question_test.jsonl
IMAGE_FOLDER=datasets/GBM_MGMT/patches
ANSWER_FILE="$LOG_DIR/z18_GBM_MGMT.jsonl"
...
echo -e "\n------------------------------Patch pathological type open ended classification------------------------------" > "$RES_FILE"
python zsxm_val/code/eval_path_cv/cls_eval.py \
    --gt $QUESTION_FILE \
    --pred $ANSWER_FILE \
    --dataset "patho_type" >> "$RES_FILE" 2>&1

echo -e "\n------------------------------Patch MGMT methylation type open ended classification------------------------------" >> "$RES_FILE"
python zsxm_val/code/eval_path_cv/cls_eval.py \
    --gt $QUESTION_FILE \
    --pred $ANSWER_FILE \
    --dataset "GBM_MGMT" >> "$RES_FILE" 2>&1
