...
RES_FILE="$LOG_DIR/14_ICC_subtype.log"
mkdir -p $LOG_DIR

# ************************************* Evaluate for Patch *************************************
QUESTION_FILE=datasets/ZheYi0607/ICC_subtype/patch_question_test.jsonl
IMAGE_FOLDER=datasets/ZheYi0607/ICC_subtype/patches
ANSWER_FILE="$LOG_DIR/z14_ICC_subtype.jsonl"
...
echo -e "\n------------------------------Patch ICC subtype close ended classification------------------------------" >> "$RES_FILE"
python zsxm_val/code/eval_path_cv/choice_eval.py \
    --gt $QUESTION_FILE \
    --pred $ANSWER_FILE \
    --dataset "ICC_subtype" >> "$RES_FILE" 2>&1
